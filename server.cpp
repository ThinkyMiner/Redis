...
struct Conn {
    ...
    int fd = -1; // fd is non-negative so this means no fd allocated
...
static Conn *handle_accept(int fd) {
    ...
    cout << "new client from " << (ip & 255) << "." << ((ip >> 8) & 255) << "." << ((ip >> 16) & 255) << "." << (ip >> 24) << ":" << ntohs(client_addr.sin_port) << endl;
...
static bool try_one_request(Conn *conn) {
    ...
    // Why not just empty the buffer? See the explanation of "pipelining".
    return true;        // success
}
